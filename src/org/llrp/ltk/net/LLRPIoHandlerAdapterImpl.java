...
import org.llrp.ltk.generated.messages.KeepaliveAck;
...
public class LLRPIoHandlerAdapterImpl extends LLRPIoHandlerAdapter{
	...
	public void messageReceived(IoSession session, Object message)
		...
		if(message instanceof Keepalive){
			...
			if(keepAliveAck){
				KeepaliveAck cc = new KeepaliveAck();
				cc.setMessageID(llrpMessage.getMessageID());
				session.write(cc);
				return;
			}
...
//		}
//		change by wuwh
		if (llrpMessage instanceof DeviceEventNotification) {
			 ConnectionAttemptEvent connectionAttemptEvent = ((DeviceEventNotification)message).getConnectionAttemptEvent();
			 if(connectionAttemptEvent != null){
				 connectionAttemptEventQueue.add(connectionAttemptEvent);
				 this.connection.setDevicesn(llrpMessage.getDevicesn());
				 connection.getEndpoint().messageReceived(llrpMessage);
				 return;
			 }
		}
