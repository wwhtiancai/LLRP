...
//	public static final int MINHEADERLENGTH = 80;
	...
	//change by wuwh
	public static final int RESERVEDLENGTH = 72;// DEVICESNLENGTH +VERSIONLENGTH 
	public static final int DEVICESNLENGTH = 64; 
	public static final int VERSIONLENGTH = 8;  
	public static final int TYPENUMBERLENGTH = 16;
	public static final int PARAMETERTYPELENGTH = 16;
	public static final int PARAMETERLENGTH = 16;
	public static final int MINHEADERLENGTH = 152;
	public final int messageReservedLength = 8;
	
	protected BitList reserved = new BitList(messageReservedLength);
	protected BitList devicesn = new BitList(DEVICESNLENGTH);
...
//			result.append(reserved.encodeBinary());
			result.append(devicesn.encodeBinary());
			if (version != null) {
...
//					messageReservedLength + DEVICESNLENGTH, TYPENUMBERLENGTH));
	...
	public final void decodeBinary(byte[] byteArray)
		...
		try {
			...
			if (bits.length() < MINHEADERLENGTH) {
				LOGGER.error("Bit String too short, must be at least 152, is "
						+ bits.length());
...
//					.toShort();
			for (int i = 0; i < DEVICESNLENGTH; i++) {
			...
			// skip first three bits as they are reserved    change by wuwh 
			int position = DEVICESNLENGTH;
			...
			// signedShort
			position += SignedShort.length()+messageReservedLength;
			...
			// change by wuwh (messagelength means the Messagevalue length not all header) 
			if (messageLength.intValue() +19==(byteArray.length) ){
	...
	/**
	 * Message devicesn to distinguish messages of same type.
	 * 
	 * @return UnsignedInteger
	 */
	public BitList getDevicesn() {
		return devicesn;
	}
	...
	private void finalizeEncode(LLRPBitList result) {
		...
		int lengthBytes = lengthBits / 8 -19;
		...
		for (int i = 0; i < binLength.length(); i++) {
			...
			if (binLength.get(i)) {
				result.set(DEVICESNLENGTH + messageReservedLength
						+ TYPENUMBERLENGTH + i);
			} else {
				result.clear(DEVICESNLENGTH + messageReservedLength
						+ TYPENUMBERLENGTH + i);
			}
		}
	}
