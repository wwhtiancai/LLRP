...
public abstract class TLVParameter extends LLRPParameter {
    ...
    protected final int RESERVEDLENGTH = 0;  //change by wuwh DELETE RESERVEDLENGTH
    protected final int TYPENUMBERLENGTH = 16;  //change 10 to  16 by wuwh  
    protected final int PARAMETERTYPELENGTH = 16;
    ...
    public final void decodeBinary(LLRPBitList binary) {
        ...
        bitLength = new UnsignedShort(8 * (byteLength + 4));
        ...
        decodeBinarySpecific(binary.subList(0,
                binary.length()));
    }
...
//        int lengthBytes = lengthBits / 8; change by wuwh
        int lengthBytes = lengthBits / 8 - 4;
        if ((lengthBits % 8) > 0) {
            lengthBytes++;
        }
